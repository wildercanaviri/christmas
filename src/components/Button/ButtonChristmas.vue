<script setup lang="ts">
 ...
 const messageButton = ref("Descubre tu mensaje motivacional");

 const apiEndpoint = 'https://deep-translate1.p.rapidapi.com/language/translate/v2';
 ...
 const fetchData = async () => {
   try {
     ...
     const messageResponse = value[0]["q"];

     const translate = await translateText(messageResponse);
     message.value = translate.translations.translatedText;
     loading.value = false;
     messageButton.value = "Otro mensaje motivacional";
     emit('message', message.value);
     emit('loading', loading.value);
   } catch (err: any) {
     message.value = "Si eres capaz de sonreir, Eres capaz de ser feliz";
     loading.value = false;
     messageButton.value = "Descubre tu mensaje motivacional";
     emit('message', message.value);
     emit('loading', loading.value);
   }
 };
 ...
 async function translateText(text: string) {
   const headers = {
     'x-rapidapi-key': 'be1a8483e8msh78b3444b2329dd9p17828djsn84e29673901a',
...
<template>
  <div class="centerer pb-24 md:pb-4">
    <button class="button" @click="handleClick()">{{ messageButton }}</button>
  </div>
